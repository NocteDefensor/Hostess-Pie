...
def split_files():
	...
	with open("./results/hostess-scope.txt", "r") as file:
		data = file.read()
...
def convert_fullscope():
	# Hostnames and CIDR ranges are converted to IP addresses (hostess-scope.txt)
	command = "nmap -sL -n -iL fullscope.txt -oG - | grep -v '#' | cut -d ' ' -f 2 | sort -u > ./results/hostess-scope.txt"
	run_command(command)

	# Hostnames are appended to hostess-scope.txt
	command = "grep -i \"[a-z]\" fullscope.txt | sort -u >> ./results/hostess-scope.txt"
	run_command(command)
...
def main():
	...
	if not os.path.exists("./domains.txt") or not os.path.exists("./fullscope.txt"):
		log("Both the 'fullscope.txt' and 'domains.txt' files must be present. Exiting.", ERROR)
		exit(1)
